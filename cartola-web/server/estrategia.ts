...
async function carregarHistoricoCompleto(anos?: number[]): Promise<Record<string, HistoricoJogador>> {
  const whereClause = anos && anos.length > 0
...
export class EstrategiaV7 {
  ...
  async carregarHistorico(): Promise<void> {
    ...
    const anos = this.modoSimulacao ? [2022, 2023, 2024] : [2022, 2023, 2024, 2025, 2026];
    const hist = await carregarHistoricoCompleto(anos);
    ...
    console.log(`[EstrategiaV7] Histórico carregado: ${Object.keys(this.historico).length} jogadores`);
  }
  ...
  async prepararFeaturesV9(jogadores: Jogador[], modoSimulacao: boolean = false): Promise<Jogador[]> {
  ...
  async escalarTime(
    ...
    const df = await this.prepararFeaturesV9(dfMercado, modoSimulacao);
    
    // Load ML predictions from Neon DB (generated by Python RandomForest model)
    try {
      ...
      let mlCount = 0;
      for (const j of df) {
        const mlPred = predMap.get(String(j.atleta_id));
        if (mlPred !== undefined && mlPred > 0) {
          j.pred = mlPred;
          j.score = mlPred;
          mlCount++;
        } else {
          j.pred = j.score_geral_v9 || 0;
          j.score = j.pred;
        }
      }
      console.log(`[ML] Loaded ${mlPreds.length} ML predictions, matched ${mlCount} jogadores`);
    } catch (err) {
      console.warn('[ML] Failed to load ML predictions, falling back to score_geral_v9:', err);
      for (const j of df) {
        j.pred = j.score_geral_v9 || 0;
        j.score = j.pred;
      }
