...
def populate_copa2026_data(session):
    ...
    Usa os campos corretos do modelo Match:
    - match_number, team1_code, team2_code, datetime, city, phase, group
    """
    from copa2026_data import ALL_MATCHES, TEAMS
    from models import Match, Team
    from datetime import datetime
    
    print("🏆 Carregando dados da Copa do Mundo 2026...")
    
    # Verificar se já existem seleções no banco
    existing_teams = session.query(Team).count()
    if existing_teams == 0:
        # Cadastrar seleções
        for code, info in TEAMS.items():
            # Determina o grupo baseado nos jogos
            team_group = None
            for match in ALL_MATCHES[:72]:  # Apenas fase de grupos
                if match[2] == code or match[3] == code:
                    team_group = match[1]  # grupo está na posição 1
                    break
            ...
            session.add(team)
        session.commit()
        print(f"✅ Cadastradas {len(TEAMS)} seleções.")
    ...
    # Buscar mapeamento de códigos para IDs
    teams_map = {t.code: t.id for t in session.query(Team).all()}
    
    # Inserir todos os 104 jogos
    matches_added = 0
    
    for match_data in ALL_MATCHES:
        # Formato da tupla: (match_number, group/phase, team1, team2, date, time, venue)
        match_number, phase_or_group, team1_code, team2_code, date_str, time_str, city = match_data
        
        # Converter data e hora para datetime
        datetime_str = f"{date_str} {time_str}"
        match_datetime = datetime.strptime(datetime_str, "%Y-%m-%d %H:%M")
        
        # Determinar fase e grupo
        # Se for letra única (A-L), é fase de grupos
        if len(phase_or_group) == 1 and phase_or_group in 'ABCDEFGHIJKL':
            phase = 'Grupos'
            group = phase_or_group
        else:
            # Mata-mata: R32, R16, QF, SF, 3RD, FINAL
            phase = phase_or_group
            group = None
        
        # Buscar IDs dos times (pode ser None se ainda não definido)
        team1_id = teams_map.get(team1_code)
        team2_id = teams_map.get(team2_code)
        
        # Criar objeto Match com os campos corretos do modelo
        match = Match(
